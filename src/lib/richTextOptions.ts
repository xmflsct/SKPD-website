  ...
  renderNode: {
    [BLOCKS.EMBEDDED_ASSET]: node => {
      ...
      if (node.data.target.fields.file.contentType.startsWith('image')) {
        ...
        const widths = [400, 782, 1200, 1600]
        const baseParams = 'q=80&fm=webp'
        const srcset = widths
          .map(w => `${url}?w=${w}&${baseParams} ${w}w`)
          .join(', ')

        return (`<figure class="my-6"><img src="${url}?w=782&${baseParams}" srcset="${srcset}" sizes="(max-width: 768px) 100vw, 782px" alt="${node.data.target.fields.title}" class="rounded-lg shadow-sm" loading="lazy"${widthAttr}${heightAttr} /></figure>`
        )
      } else {
